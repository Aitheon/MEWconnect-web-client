...
mewConnectClient.on('codeDisplay', code => {
  ...
  mewConnectClient.on('RtcConnectedEvent', function () {
    ...
    const messageToVerify = 'Aitheon';
    const msgHash = MewConnect.Utils.hashPersonalMessage()();
    mewConnectClient.sendRtcMessage('signMessage', {
        hash: msgHash.toString('hex'),
        text: messageToVerify
    });

    mewConnectClient.once('signMessage', data => {
